...
| 知识搜索 | `knowledge_search` | 从知识库中检索相关信息 |
| 知识阅读 | `knowledge_read` | 读取知识库中的具体内容 |
...
ReactLoop 在 system prompt 中注入 **ReAct Protocol**，指导模型遵循规范的循环行为：
...
2. Action: 调用恰好一个工具
3. Observation: 接收工具输出，用于下一轮 Thought

完成条件：当本步骤目标达成时，直接回复最终答案，不再调用工具
错误处理：工具失败时在 Thought 中分析替代方案
约束：只执行当前步骤，不涉及后续步骤
```
...
**System Prompt 组装**（按顺序拼接）：

| 层级 | 内容 | 来源 |
|------|------|------|
| 1 | 系统提示词（角色定位 + 行为规范） | AgentConfig.systemPrompt |
| 2 | ReAct 行为协议 | REACT_PROTOCOL 常量 |
| 3 | 思维模式引导 | ThinkingModeScheduler |
| 4 | 执行计划概览（所有步骤及依赖） | Plan |
| 5 | 当前步骤说明 | PlanStep |
| 6 | 已安装技能目录 | SkillHub.getSkillsDescription() |
| 7 | 记忆上下文 | MemoryHub.searchMemory() |
